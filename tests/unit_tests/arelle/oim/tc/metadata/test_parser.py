...
from arelle.oim.tc.const import (
    TC_NS_CR,
    TC_NS_DRAFT,
    TC_PREFIX,
...
from arelle.oim.tc.metadata.model import TCValueConstraint
from arelle.oim.tc.metadata.parser import TCMetadataMissingPropertiesError, parse_tc_metadata
...
class TestNamespaceDetection:
    ...
    def test_detects_cr_namespace(self) -> None:
        result = parse_tc_metadata({"tableTemplates": {}}, {"tc": TC_NS_CR})
        assert result.metadata is not None
...
class TestPrimitiveStr:
    def test_present_correct_type(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": "xs:integer"}), TC_MINIMAL_NAMESPACES)
        assert result.metadata is not None
    ...
    def test_default_when_missing(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": "xs:string"}), TC_MINIMAL_NAMESPACES)
        assert result.metadata is not None
        assert result.metadata.template_constraints["t1"].constraints["col"].period_type is None

    def test_json_null_raises(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": None}), TC_MINIMAL_NAMESPACES)
        assert not result.is_valid


class TestPrimitiveBool:
    def test_present_correct_type(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": "xs:string", "optional": True}), TC_MINIMAL_NAMESPACES)
        assert result.metadata is not None
    ...
    def test_json_null_raises(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": "xs:string", "optional": None}), TC_MINIMAL_NAMESPACES)
        assert not result.is_valid


class TestPrimitiveInt:
    def test_present_correct_type(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": "xs:string", "length": 10}), TC_MINIMAL_NAMESPACES)
        assert result.metadata is not None
    ...
    def test_bool_true_rejected(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": "xs:string", "length": True}), TC_MINIMAL_NAMESPACES)
        assert not result.is_valid

    def test_bool_false_rejected(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": "xs:string", "length": False}), TC_MINIMAL_NAMESPACES)
        assert not result.is_valid

    def test_json_null_raises(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": "xs:string", "length": None}), TC_MINIMAL_NAMESPACES)
        assert not result.is_valid

    def test_zero_accepted_for_non_negative(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": "xs:string", "length": 0}), TC_MINIMAL_NAMESPACES)
        assert result.metadata is not None
    ...
    def test_negative_rejected_for_non_negative(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": "xs:string", "length": -1}), TC_MINIMAL_NAMESPACES)
        assert not result.is_valid
...
class TestValueConstraint:
    def test_minimal_defaults(self) -> None:
        result = parse_tc_metadata(_with_constraint({"type": "xs:date"}), TC_MINIMAL_NAMESPACES)
        assert result.metadata is not None
...
class TestOrderedSetFields:
    ...
    def test_duplicates_raise(self) -> None:
        result = parse_tc_metadata(_with_template({"tc:columnOrder": ["a", "a"]}), TC_MINIMAL_NAMESPACES)
        assert not result.is_valid

    def test_non_list_raises(self) -> None:
        result = parse_tc_metadata(_with_template({"tc:columnOrder": "not a list"}), TC_MINIMAL_NAMESPACES)
        assert not result.is_valid

    def test_non_string_element_raises(self) -> None:
        result = parse_tc_metadata(_with_template({"tc:columnOrder": ["a", 1]}), TC_MINIMAL_NAMESPACES)
        assert not result.is_valid
...
class TestEdgeCases:
    def test_missing_table_templates(self) -> None:
        result = parse_tc_metadata({}, TC_MINIMAL_NAMESPACES)
        assert result.metadata is not None
        assert result.metadata.template_constraints == {}

    def test_empty_table_templates(self) -> None:
        result = parse_tc_metadata({"tableTemplates": {}}, TC_MINIMAL_NAMESPACES)
        assert result.metadata is not None
        assert result.metadata.template_constraints == {}
