...
def test_open_csv_reader_closes_the_file_after_iteration() -> None:
    ...
    def file(filepath: str, binary: bool = False, encoding: str | None = None) -> tuple[IO[Any]]:
        handle: IO[Any] = io.BytesIO(b"a,b\n1,2\n") if binary else io.StringIO("a,b\n1,2\n")
        handles.append(handle)
        return (handle,)

    file_source = Mock(spec=FileSource, file=file)
    assert list(openCsvReader(file_source, "t.csv", CSV_FACTS_FILE)) == [["a", "b"], ["1", "2"]]
