"""
Confirms the command line example published on the website still produces the output published with it.
"""
...
command_path = examples_directory / "cli-validation-command.txt"
output_path = examples_directory / "cli-validation-output.txt"
...
command_text = command_path.read_text().strip()
command_line = re.sub(
    r"\\\r?\n[ \t]*",
    "",
    command_text,
...
example_args = [
    ...
    for argument in shlex.split(command_line.removeprefix(COMMAND_PREFIX))
]
