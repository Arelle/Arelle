...
ALL_INVALID_ERRORS = dict(VALID_EXPECTED_ERRORS)
for invalid_id, valid_id in INVALID_TESTCASE_PARENTS.items():
    errors = dict(VALID_EXPECTED_ERRORS.get(valid_id, {}))
    additional = ADDITIONAL_INVALID_ERRORS.get(invalid_id, {})
    for k, v in additional.items():
        errors[k] = errors.get(k, 0) + v
    ALL_INVALID_ERRORS[invalid_id] = errors
...
config = ConformanceSuiteConfig(
    ...
    expected_additional_testcase_errors={f"*{s}": val for s, val in ALL_INVALID_ERRORS.items()},
