...
from arelle.PythonUtil import OrderedDefaultDict, OrderedSet, isRootedPath
...
def loadFromExcel(cntlr, modelXbrl, excelFile, mappedUri):
    ...
    def docRelpath(filename, baseDir=None):
        ...
        if (baseDir is not None and
            not (UrlUtil.isAbsolute(filename) or isRootedPath(filename))):
        ...
        try:
            ...
            if action == "import":
                ...
                if filetype == "schema":
                    ...
                    if not UrlUtil.isAbsolute(filename) and not isRootedPath(filename):
...
def saveDts(cntlr, modelXbrl, outputDtsDir):
    ...
    for doc in modelXbrl.urlDocs.values():
        ...
        elif not (UrlUtil.isAbsolute(doc.uri) or isRootedPath(doc.uri) or outputDtsDir == excelFileDir):
