...
def saveOIMFactPositions(
    ...
    **kwargs: Any,
    ...
    def factFootnotes(
        fact: ModelFact,
        factPosition: dict[str, Any] | None = None,
    ):
        footnotes = []
        oimLinks = {}
        if saveOimJson:
            ...
            if oimLinks:
                ...
                if isJSON and factPosition is not None:
                    factPosition["links"] = _links
        else:
            for footnoteRel in footnotesRelationshipSet.fromModelObject(fact):
    ...
    def factDimensions(fact: ModelFact, factPositions, factspacesByDims) -> dict[str, Any]:
        ...
        if saveOimJson:
            factPositions[f"{fact.id or fact.objectIndex}"] = factPosition = {}
            factPosition["dimensions"] = factDims
        else:
            # build dims key
            dimsKey = tuple(sorted(factDims.items(), key=lambda k:k[0]))
            factPosition = factspacesByDims.get(dimsKey)
            if factPosition is None or saveOimJson:
                factPosition = {}
                factPosition["name"] = f"{getPrefix(fact)}:fs_{fact.id or fact.objectIndex}"
                factPosition["factDimensions"] = factDims
                factPosition["factValues"] = []
                factspacesByDims[dimsKey] = factPosition
                factPositions.append(factPosition)
        return factPosition
    
    def appendFactToFactspace(fact: ModelFact, factPosition: dict[str, Any]) -> None:
        if fact.concept is not None and fact.isItem:
            if saveOimJson:
                factValue = factPosition
        ...
        if not saveOimJson:
            factPosition["factValues"].append(factValue)
    ...
    # save JSON
    if saveOimJson:
        oimModel["facts"] = factPositions = {}
    else:
        oimModel["xbrlModel"]["factPositions"] = factPositions = []
    factspacesByDims = {} # unique entry for mutli-valued factPositions
    ...
    def saveJsonFacts(facts: list[ModelFact]) -> None:
        for fact in facts:
            factPosition = factDimensions(fact, factPositions, factspacesByDims)
            appendFactToFactspace(fact, factPosition)
            # add in fact level extension objects
            if extensionFactPropertiesMethod:
                extensionFactPropertiesMethod(fact, oimFact)
            factFootnotes(fact, factPosition)
...
def SaveOIMFactspaceMenuCommand(cntlr: CntlrWinMain) -> None:
    ...
    thread = threading.Thread(
        target=lambda _modelXbrl=cntlr.modelManager.modelXbrl, _oimFile=oimFile: saveOIMFactPositions(
    ...
    try:
        if responseZipStream is None:
            for oimFile in oimFiles:
                saveOIMFactPositions(modelXbrl, oimFile, None, saveInlineTextValue, saveOimJson)
        else:
            with zipfile.ZipFile(responseZipStream, "a", zipfile.ZIP_DEFLATED, True) as _zip:
                for oimFile in oimFiles:
                    saveOIMFactPositions(modelXbrl, oimFile, _zip, saveInlineTextValue, saveOimJson)
...
class SaveOIMFactspacePlugin(PluginHooks):
    ...
    def cntlrCmdLineXbrlRun(
        ...
        **kwargs: Any,
        ...
        oimFile = cast(Optional[str], getattr(options, "saveOIMFactPositions", None))
    ...
    def testcaseVariationValidated(
        ...
        **kwargs: Any,
        ...
        if (
            ...
            try:
                saveOIMFactPositions(testInstanceDTS, testInstanceDTS.modelDocument.uri + oimFileSuffix)
            except Exception as ex:
                testcaseDTS.modelManager.cntlr.addToLog(f"Exception saving OIM {ex}")

    @staticmethod
    def saveOIMFactPositionSave(
        modelXbrl: ModelXbrl,
        oimFile: str,
        *args: Any,
        **kwargs: Any,
    ) -> None:
        return saveOIMFactPositions(modelXbrl, oimFile, *args, **kwargs)
...
__pluginInfo__ = {
    ...
    "SaveOIMFactspace.Save": SaveOIMFactspacePlugin.saveOIMFactPositionSave,
}
