...
class XbrlReference(XbrlTaxonomyObject):
    taxonomy: XbrlTaxonomyType
    name: QName # (required if no extendTargetame) The name is a QName that uniquely identifies the reference object.
...
class XbrlReferenceType(XbrlTaxonomyObject):
    taxonomy: XbrlTaxonomyType
    name: QName # (required) The name is a QName that uniquely identifies the reference type object.
    ...
    requiredPropertiesproperties: OrderedSet[XbrlProperty] # (optional) Defines an ordered set of property QNames that must be included within a defined reference type.