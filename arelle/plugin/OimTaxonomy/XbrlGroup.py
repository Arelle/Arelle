...
class XbrlGroup(XbrlTaxonomyObject):
    taxonomy: XbrlTaxonomyType
    name: QName # (required) The group object QName that identifies the group so it can be referenced by other objects.
...
class XbrlGroupContent(XbrlTaxonomyObject):
    taxonomy: XbrlTaxonomyType
    groupName: AnyURI # (required) The QName of the group object.
