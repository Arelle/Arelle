...
from .XbrlTypes import QNameKeyType, XbrlTaxonomyModuleType, DefaultTrue, DefaultFalse, DefaultZero
...
def saveableObjects(mdlObj, mdlName, **kwargs):
    ...
    if isinstance(mdlObj, XbrlTaxonomyModule):
    ...
    for propName, propType in type(mdlObj).propertyNameTypes():
        ...
        if propType == XbrlTaxonomyModuleType: # first prop which references parent
            continue
