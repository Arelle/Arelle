...
Where each error prefix is defined (authoritative local sources):
  ...
  oimtce:            -> oim-taxonomy/spec-taxonomies/oimtce.json     (member "name" values)
...
# oimtce codes are defined by the calculation error taxonomy rather than by prose in
...
OIMTC_JSON = os.path.join(SPECS, "oim-taxonomy/spec-taxonomies/oimtce.json")
...
oimtce = json.load(open(OIMTC_JSON))
oimtceLocalNames = {m["name"].split(":", 1)[1]
                   for m in oimtce["xbrlModel"].get("members", ()) if m.get("name")}
...
def isDefined(code):
    ...
    if prefix == "oimtce":
        return localName in oimtceLocalNames
...
def definedElsewhere(localName):
    ...
    if localName in oimtceLocalNames:
        hints.append(f"oimtce:{localName}")
...
for code in sorted(codeFiles):
    if code.split(":", 1)[0] in ("oimte", "oime", "oimce", "oimtce", "calc11e") and not isDefined(code):
        undefinedByPrefix[code.split(":", 1)[0]].append(code)

print(f"Checked {TEST_INDEX}")
total = 0
for prefix in ("oimte", "oime", "oimce", "oimtce", "calc11e"):
