...
class XbrlImportTaxonomy(XbrlModelObject):
    ...
    excludeGroupTree: Union[bool, DefaultFalse] # (optional) If set to true, the groupTree object of the imported model is excluded from the XBRL Model (the groupTree is otherwise imported automatically with the model). The default value is false.
    bundleManifest: Optional[dict] # (optional) Metadata about a label bundle or reference bundle being imported: language (set of BCP 47 codes) and bundleTypes (set of label/reference type QNames) provided by the bundle. Modeled as a plain object (no OIM object-type QName), so kept as a dict.
...
class XbrlFinalTaxonomy(XbrlReferencableModelObject):
    ...
    finalObjectTypes: Optional[NonemptySet[QName]] # (optional) A set of object type QNames that define the types of objects that are considered final. Only the object types defined in the finalObjectTypes property are considered final. Object types not included in the set can be add by an importing taxonomy. (Assuming finalTaxonomyFlag is false). An importing taxonomy cannot include object types that are defined in the finalObjectTypes property. Examples of object types include xbrl:conceptObject, xbrl:domainObject and xbrl:networkObject. Networks used for footnotes are excluded from this restriction.
    finalObjects: Optional[NonemptySet[QName]] # (optional) A set of object QNames that define the specific extendable objects that are considered final. Only the objects defined in the finalObjects property are considered final. Objects not included in the set can be added to by an importing taxonomy. (Assuming finalTaxonomyFlag is false). Extendable objects are those objects that can be extended using the extend property such as domain objects, network objects and reference objects. Note: a domain can be marked as final on the domain object. Networks used for footnotes are excluded from this restriction.
    selections: Optional[NonemptySet[XbrlStructuredSelectStatement]]  # (optional) Specifies a set of string select statements or structured select objects that define the objects to be identified as final. Each select statement is a string that specifies the objects to be selected from the taxonomy model. Selections that include objects that cannot be extended are ignored.
