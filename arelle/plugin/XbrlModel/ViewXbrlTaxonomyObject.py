    ...
    def view(self):
        ...
        for obj in self.xbrlCompMdl.filterNamedObjects(self.objClass, role, lang): # this is a yield generator
            ...
            node = self.treeView.insert("", "end",
                                        ...
                                        text=str(self.xbrlCompMdl.labelValue(obj.getProperty(propName, self.labelrole), self.labelrole, self.lang)),
        ...
        if nestedObjs:
            for propName, propType in obj.propertyNameTypes():
                ...
                if isinstance(getattr(propType, "__origin__", None), type(Union)): # Optional[ ] type
                    if isinstance(propType.__args__[0], XbrlModelObject): # e.g. dateResolution object
                        childObj = getattr(obj, propName, None)
                        if childObj is not None:
                            self.viewProps(node, nodeNum, childObj)
                elif getattr(propType, "__origin__", None) in (set, OrderedSet) and issubclass(propType.__args__[0], XbrlModelObject):
