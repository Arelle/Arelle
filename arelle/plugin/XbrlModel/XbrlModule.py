...
class XbrlImpliedObject(XbrlReferencableModelObject):
    ...
    localNameConstraint: Optional[QName] # (optional) A reference to a datatype object that defines constraints on the local-name of the implied object.
...
class XbrlModule(XbrlModelObject):
    ...
    referenceModel: Optional[QName] # (optional) A QName that references a XBRL model object. The referenceModel property is only used with a model that has a documentType of https://xbrl.org/2026/labelBundle or https://xbrl.org/2026/referenceBundle.
