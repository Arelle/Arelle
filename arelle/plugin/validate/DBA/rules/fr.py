            ...
            if fact.xValid >= VALID:
                if fact.xValue in [
                    ...
                    if len(missing_concepts) > 0:
                        yield Validation.warning(
                            codes='DBA.FR53',
                            msg=_(f"The following concepts must be tagged: {','.join(missing_concepts)} when "
                                  f"{pluginData.typeOfAuditorAssistanceQn.localName} is tagged with the value of {fact.xValue}"),
                            modelObject=fact
                        )
