...
from ..Manifest import ManifestTocItem
from ..PluginValidationDataExtension import PluginValidationDataExtension

_: TypeGetText


def _validateTocItem(modelXbrl: ModelXbrl, tocItem: ManifestTocItem, errors: list[Validation]) -> None:
    for childTocItem in tocItem.childItems:
        _validateTocItem(modelXbrl, childTocItem, errors)
...
def rule_EC5800E(
    ...
    """
    manifest = pluginData.getManifest(val.modelXbrl)
    if manifest is None:
        return
    errors: list[Validation] = []
    for tocItem in manifest.tocItems:
        _validateTocItem(val.modelXbrl, tocItem, errors)
    yield from errors
