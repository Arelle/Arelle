...
def parseArgs(args):
    ...
    parser.add_option("--validateDuplicateFacts", "--validateduplicatefacts",
                      choices=[x.toArg() for x in ValidateDuplicateFacts.DisallowMode],
...
class CntlrCmdLine(Cntlr.Cntlr):
    ...
    def run(self, options, sourceZipStream=None, responseZipStream=None):
        ...
        if options.validateDuplicateFacts:
            disallowMode = ValidateDuplicateFacts.DisallowMode.fromArg(options.validateDuplicateFacts)
            self.modelManager.validateDuplicateFacts = disallowMode
