...
class Md5Sum:
    MAXMd5SUM: int = 0xffffffffffffffffffffffffffffffff

    def __init__(self, initialValue: int | str = 0) -> None:
        if isinstance(initialValue, int):
            self.value: int = initialValue & Md5Sum.MAXMd5SUM
    ...
    def __add__(self, other: Union["Md5Sum", int, str]) -> Md5Sum:
...
HashArg: TypeAlias = Union[Md5Sum, QName, str, datetime, date, ModelObject]


def md5hash(argList: HashArg | Iterable[HashArg]) -> Md5Sum:
    if not isinstance(argList, (list, tuple, set)):
        argList = (argList,)  # type: ignore[assignment]
    _md5 = md5()
    nestedSum = MD5SUM0
    firstMd5arg = True
    for _arg in argList:  # type: ignore[union-attr]
