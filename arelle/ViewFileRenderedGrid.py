...
from arelle.ModelRenderingObject import OPEN_ASPECT_ENTRY_SURROGATE
...
def viewRenderedGrid(
    modelXbrl: ModelXbrl,
    outfile: str | FileNamedStringIO,
    lang: str | None = None,
    ...
    if sourceView is not None:
        ...
        layoutTable(view, table)  # type: ignore[no-untyped-call]
...
class ViewRenderedGrid(ViewFile.View):
    ...
    tableModelNamespace: str
    openBreakdownLines: int
        ...
        for lytMdlTableSet in lytMdlTblMdl.lytMdlTableSets:
            ...
            for lytMdlTable in lytMdlTableSet.lytMdlTables:
                ...
                if lytMdlZHdrs is not None:
                    ...
                    zHdrElts: list[OrderedDict[tuple[QName | None, ...], tuple[str, ...]]] = [OrderedDict() for _ in range(numZtbls)]
                        ...
                        for lytMdlHeader in lytMdlGroup.lytMdlHeaders:
                            ...
                            for iLabel in range(lytMdlHeader.maxNumLabels):
                                ...
                                for lytMdlCell in lytMdlHeader.lytMdlCells:
                                    ...
                                    attrib = {
                                        "style": "max-width:100em;",
                                        "class": "xAxisSpanLeg" if lytMdlCell.rollup else "xAxisHdr",
                                    }
    ...
    def viewXLSX(self, lytMdlTblMdl: LytMdlTableModel) -> None:
        ...
        for lytMdlTableSet in lytMdlTblMdl.lytMdlTableSets:
            ...
            titleCell = self.xlsxWs.cell(row=self.xlsxRow+1, column=1)  # type: ignore[union-attr]
            ...
            for lytMdlTable in lytMdlTableSet.lytMdlTables:
                ...
                if lytMdlZHdrs is not None:
                    lytMdlZHdrGroups = lytMdlZHdrs.lytMdlGroups
                    numZtbls = lytMdlTable.numBodyCells("z") or 1  # must have at least 1 z entry
                    zHdrLbls: list[list[list[str]]] = [[] for _ in range(numZtbls)]
                    for lytMdlZGrp in lytMdlZHdrGroups:
                        for lytMdlZHdr in lytMdlZGrp.lytMdlHeaders:
                            zRow = 0
                            if all(lytMdlCell.isOpenAspectEntrySurrogate for lytMdlCell in lytMdlZHdr.lytMdlCells):
                ...
                for lytMdlYBodyCell in lytMdlZBodyCell.lytMdlBodyChildren:  # type: ignore[union-attr]
                    ...
                    zHdrCell = self.xlsxWs.cell(row=yFirstHdrRow, column=1)  # type: ignore[union-attr]
                        ...
                        for lytMdlHeader in lytMdlGroup.lytMdlHeaders:
                            ...
                            for iLabel in range(lytMdlHeader.maxNumLabels):
                                ...
                                for i, lytMdlCell in enumerate(lytMdlHeader.lytMdlCells):
                                    ...
                                    if lytMdlCell.span > 1:
                                        self.xlsxWs.merge_cells(range_string="%s%s:%s%s" % (utils.get_column_letter(xlsxCol), self.xlsxRow, utils.get_column_letter(xlsxCol + lytMdlCell.span - 1), self.xlsxRow))  # type: ignore[union-attr]
                                        xlsxCol += lytMdlCell.span
                                    else:
                                        xlsxCol += 1
                    # upper left row/col hdr col
                    if yHdrCols > 1:
                        self.xlsxWs.merge_cells(range_string="%s%s:%s%s" % (utils.get_column_letter(1), yFirstHdrRow, utils.get_column_letter(yHdrCols), self.xlsxRow))  # type: ignore[union-attr]
                    yRowNum = 0
                    ...
                    for lytMdlXBodyCell in lytMdlYBodyCell.lytMdlBodyChildren:  # type: ignore[union-attr]
                        ...
                        if yRowNum < len(yRowHdrs):
                            for i, rowHdrElt in enumerate(yRowHdrs[yRowNum]):
                                ...
                                if rowspan > 1:  # type: ignore[operator]
                                    rowspans[xlsxCol-1] = rowspan - 1  # type: ignore[operator]
                                    self.xlsxWs.merge_cells(range_string="%s%s:%s%s" % (utils.get_column_letter(xlsxCol), self.xlsxRow, utils.get_column_letter(xlsxCol), self.xlsxRow + rowspan - 1))  # type: ignore[union-attr, operator]
                                xlsxCol += 1
                        for lytMdlCell in lytMdlXBodyCell.lytMdlBodyChildren:  # type: ignore[union-attr]
                            if lytMdlCell.isOpenAspectEntrySurrogate:  # type: ignore[union-attr]
                                xlsxCol += 1
                            ...
                            if len(lytMdlCell.facts) == 0:  # type: ignore[union-attr]
                                v = None
                            elif len(lytMdlCell.facts) == 1:  # type: ignore[union-attr]
                                v = lytMdlCell.facts[0][1]  # type: ignore[union-attr]
                            else:
                                v = "\n".join(v for f, v, justify in lytMdlCell.facts)  # type: ignore[union-attr]
                            c = self.xlsxWs.cell(row=self.xlsxRow, column=xlsxCol)  # type: ignore[union-attr]
                            if v is not None:
            ...
            if numCols > 1:
                ...
                self.xlsxWs.merge_cells(range_string="%s%s:%s%s" % (utils.get_column_letter(1), titleXlsRow, utils.get_column_letter(numCols - 1), titleXlsRow))  # type: ignore[union-attr]
