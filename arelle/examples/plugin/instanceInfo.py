...
def showInfo(cntlr, options, modelXbrl, _entrypoint, *args, **kwargs):
    ...
    for f in modelXbrl.factsInInstance:
        ...
        if concept is not None:
            if concept.isNumeric:
                numNumericFacts += 1
            modelType = modelXbrl.qnameTypes.get(concept.typeQname)
            if modelType is not None:
                if modelType.isOimTextFactType:
                    numTextFacts += 1
                    if f.xValue:
                        lenTextFacts += len(f.xValue)
                    else:
                        lenTextFacts += len(xmlstring(f,stripXmlns=True)) # multi-doc may be missing fact definition
                if modelType.isTextBlock:
                    numTextBlockFacts += 1
                    if f.xValue:
                        lenTextBlockFacts += len(f.xValue)
                    else:
                        lenTextBlockFacts += len(xmlstring(f,stripXmlns=True))
    ...
    cntlr.addToLog("Number of numeric facts {:,}".format(numNumericFacts), messageCode="info", level=logging.DEBUG)
    cntlr.addToLog("Number of text facts {:,} total len {:,}".format(numTextFacts, lenTextFacts), messageCode="info", level=logging.DEBUG)
        ...
        if len(hashEquivalentFacts) > 1:
            for f in hashEquivalentFacts:
                aspectEqualFacts[(f.qname,f.contextID,f.unitID,
                                  f.xmlLang.lower() if f.concept.type.isWgnStringFactType and f.xmlLang else None)].append(f)
            for fList in aspectEqualFacts.values():
                f0 = fList[0]
                if f0.concept.isNumeric:
                    if any(f.isNil for f in fList):
                        _inConsistent = not all(f.isNil for f in fList)
                    elif f0.xValue is not None: # not all have same decimals
                        ...
                        for f in fList[1:]:
                            ...
                            _v = f.xValue
                            if _v is not None and isnan(_v):
                                _inConsistent = True
                                break
                            if _d in decVals:
    ...
    for ixdsHtmlRootElt in getattr(modelXbrl, "ixdsHtmlElements", ()): # ix root elements if inline
        for ixElt in ixdsHtmlRootElt.iterdescendants():
            ...
            if style:
                ...
                if "content-visibility" in style:
                    numContVisibSects += 1
                    if "contain-intrinsic-size" in style:
                        numWithContIntrSize += 1
                    containedDomNodes = sum(1 for elt in ixElt.iterdescendants())
                    if containedDomNodes > maxDomNodesPerSect:
                        maxDomNodesPerSect = containedDomNodes
    ...
    cntlr.addToLog("Number of source DOM nodes {:,}".format(numDomNodes), messageCode="info", level=logging.DEBUG)
    cntlr.addToLog("Number of content-visibility styles {:,} with contain-intrinsic-style {:,} max DOM nodes per section {:,}".format(numContVisibSects, numWithContIntrSize, maxDomNodesPerSect), messageCode="info", level=logging.DEBUG)
