...
def expectedInstanceLoaded(expectedInstance, outputInstanceToCompare):
    for f in expectedInstance.facts:
        if f.isMultiLanguage and f.xValue and "http://www.w3.org/1999/xhtml" in f.xValue:
            f.setProperty("xValue", re.sub("(<[^>]+)\s+xmlns=[\"']http://www.w3.org/1999/xhtml[\"']",r"\1", f.xValue))
